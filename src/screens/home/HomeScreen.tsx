...
// ─── Config visuels par catégorie (remplace les vraies photos) ────

const CAT_CONFIG: Record<string, { bg: [string, string]; icon: React.ComponentProps<typeof Feather>['name']; label: string }> = {
...
function getCat(cat: string) {
  return CAT_CONFIG[cat] ?? { bg: ['#1F2937', '#111827'] as [string, string], icon: 'file-text' as React.ComponentProps<typeof Feather>['name'], label: cat };
}
...
const JaHeroCard: React.FC<{ item: HeroData; onPress: () => void }> = ({ item, onPress }) => {
  const cat = getCat(item.category);
  const badge = getBadge(item.articleType);
  return (
    <TouchableOpacity onPress={onPress} activeOpacity={0.9} style={heroS.wrapper}>
      {/* Fond coloré simulant la photo */}
      <LinearGradient colors={cat.bg} style={heroS.image} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }}>
      ...
      >
        ...
        <Text style={heroS.title} numberOfLines={3}>{item.title}</Text>
      </LinearGradient>

      {/* Barre catégorie + icônes (comme JA) */}
      <View style={heroS.footer}>
        <Text style={heroS.catLabel}>{cat.label}</Text>
        <View style={heroS.footerIcons}>
          {item.hasAudio && <Feather name="volume-2" size={18} color="rgba(255,255,255,0.75)" />}
...
const heroS = StyleSheet.create({
  ...
  footerIcon: { fontSize: 18, opacity: 0.8 },
});

// ─── Carte article grille 2 colonnes (style JA) ───────────────────

const JaGridCard: React.FC<{ article: Article; onPress: () => void }> = ({ article, onPress }) => {
  const cat = getCat(article.category as string);
  const badge = getBadge(article.articleType);
  return (
    <TouchableOpacity onPress={onPress} activeOpacity={0.85} style={[gridS.card, { width: CARD_W }]}>
      {/* Image + badge */}
      ...
      {/* Titre sous l'image */}
      <Text style={gridS.title} numberOfLines={3}>{article.title}</Text>

      {/* Footer : catégorie + icônes */}
      <View style={gridS.footer}>
        <Text style={gridS.catLabel} numberOfLines={1}>{cat.label}</Text>
        <View style={gridS.footerIcons}>
          {article.hasAudio && <Feather name="volume-2" size={14} color="rgba(255,255,255,0.55)" />}
...
const MostReadSection: React.FC<{ articles: Article[]; onPress: (id: string) => void }> = ({ articles, onPress }) => (
  <View style={mrS.container}>
    ...
    <FlatList
      ...
      renderItem={({ item, index }) => (
        <TouchableOpacity style={mrS.card} onPress={() => onPress(item.id)} activeOpacity={0.8}>
          <Text style={mrS.rank}>{index + 1}</Text>
          <View style={mrS.cardContent}>
            <CategoryBadge category={item.category} />
            <Text style={mrS.cardTitle} numberOfLines={3}>{item.title}</Text>
...
const mrS = StyleSheet.create({
  container: { backgroundColor: Colors.backgroundCard, borderTopWidth: 1, borderTopColor: Colors.borderLight, paddingVertical: Spacing['3'] },
  header: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: Spacing['4'], marginBottom: Spacing['3'], gap: Spacing['2'] },
  title: { fontFamily: FontFamily.headingBold, fontSize: FontSize.lg, color: Colors.textPrimary },
  list: { paddingHorizontal: Spacing['4'], gap: Spacing['3'] },
  card: { width: 160, backgroundColor: Colors.background, borderRadius: Radius.md, padding: Spacing['3'], gap: Spacing['2'] },
  rank: { fontFamily: FontFamily.headingBold, fontSize: FontSize['2xl'], color: Colors.primaryUltraLight, letterSpacing: -1, lineHeight: FontSize['2xl'] },
  cardContent: { gap: Spacing['1'] },
  cardTitle: { fontFamily: FontFamily.headingSemiBold, fontSize: FontSize.sm, color: Colors.textPrimary, lineHeight: FontSize.sm * 1.4 },
  cardDate: { fontFamily: FontFamily.body, fontSize: FontSize.xs, color: Colors.textMuted },
});

// ─── Filtre catégories ────────────────────────────────────────────

const CategoryFilter: React.FC<{ selected: Category | null; onSelect: (cat: Category | null) => void }> = ({ selected, onSelect }) => (
  <ScrollView horizontal showsHorizontalScrollIndicator={false} style={filterS.row} contentContainerStyle={filterS.content}>
    <TouchableOpacity style={[filterS.chip, !selected && filterS.chipActive]} onPress={() => onSelect(null)}>
      <Text style={[filterS.chipText, !selected && filterS.chipTextActive]}>Tous</Text>
    </TouchableOpacity>
    {CATEGORY_OPTIONS.map(({ value, label }) => (
      <TouchableOpacity key={value} style={[filterS.chip, selected === value && filterS.chipActive]} onPress={() => onSelect(value)}>
        <Text style={[filterS.chipText, selected === value && filterS.chipTextActive]}>{label}</Text>
      </TouchableOpacity>
    ))}
  </ScrollView>
);
...
const filterS = StyleSheet.create({
  row: { backgroundColor: Colors.backgroundCard, borderBottomWidth: 1, borderBottomColor: Colors.borderLight },
  content: { paddingHorizontal: Spacing['4'], paddingVertical: Spacing['3'], gap: Spacing['2'] },
  chip: { paddingHorizontal: Spacing['3'], paddingVertical: Spacing['2'], borderRadius: Radius.full, backgroundColor: Colors.background, borderWidth: 1, borderColor: Colors.border },
  chipActive: { backgroundColor: Colors.primary, borderColor: Colors.primary },
  chipText: { fontFamily: FontFamily.bodySemiBold, fontSize: FontSize.sm, color: Colors.textSecondary },
...
export const HomeScreen: React.FC<HomeScreenProps> = ({
  ...
  const filteredArticles = useMemo(
    () => selectedCategory ? MOCK_ARTICLES.filter((a) => a.category === selectedCategory) : MOCK_ARTICLES,
    [selectedCategory],
  );

  const showRating = !ratingDismissed && shouldShowRatingWidget(articlesRead);

  // ── Feed "À la une" : hero + grille 2 colonnes (style JA) ──
  const uneFeed = useMemo<FeedItem[]>(() => {
    const items: FeedItem[] = [
      { type: 'hero', data: MOCK_HERO[0] },
    ];

    // Paires d'articles (grille 2 colonnes)
    const gridArticles = [...MOCK_HERO.slice(1).map((h) => ({
      id: h.id,
      title: h.title,
  ...
  // ── Feed "Tous les articles" : filtre + liste verticale ──
  ...
  const renderItem: ListRenderItem<FeedItem> = useCallback(({ item }) => {
    switch (item.type) {
      ...
      case 'hero':
        return <JaHeroCard item={item.data} onPress={() => handleArticlePress(item.data.id)} />;

      case 'article_pair':
        return (
          <View style={styles.pair}>
            <JaGridCard article={item.left}  onPress={() => handleArticlePress(item.left.id)} />
            {item.right
              ? <JaGridCard article={item.right} onPress={() => handleArticlePress(item.right!.id)} />
      ...
      case 'most_read':
        return <MostReadSection articles={MOCK_PLUS_LUS} onPress={handleArticlePress} />;
      ...
      case 'category_filter':
        return <CategoryFilter selected={selectedCategory} onSelect={setSelectedCategory} />;

      case 'article':
        return <ArticleCard article={item.data} onPress={() => handleArticlePress(item.data.id)} />;

      default:
        return null;
    }
  }, [handleArticlePress, selectedCategory]);
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Strip animé — deux FlatLists côte à côte */}
      <View style={styles.tabStrip} {...swipe.panHandlers}>
        <Animated.View style={[styles.tabContent, { transform: [{ translateX: tabTx }] }]}>
          <FlatList
            style={{ width: W }}
            data={uneFeed}
...
const styles = StyleSheet.create({
  ...
  pillText: { fontFamily: FontFamily.bodySemiBold, fontSize: FontSize.sm, color: Colors.textMuted },
